...
impl AxiomBuilderState {
    ...
    ///
    /// Premise-side existentials are lifted to outer universals via
    /// `(∃ x. P(x)) → Q ≡ ∀ x. (P(x) → Q)`. This identity is only sound when `x` is bound in the
    /// antecedent — checked below.
    fn build_branch_axiom_for(
        ...
        let antecedent = Proposition::optional_conjunction(steps);
        let conclusion = Proposition::build_relation_predicate(&prepared.binding, RESULT_PARAM);

        let antecedent_vars = antecedent.collect_variables();
        for p in &additional_parameters {
            if p.quantifier == Quantifier::Existential {
                assert!(
                    antecedent_vars.contains(&p.name),
                    "Existential parameter `{}` in branch {} of `{}` does not appear in the antecedent; \
                     lifting (∃x.P)→Q ≡ ∀x.P→Q would falsify the axiom",
                    p.name,
                    idx,
        ...
        let lifted_params: Vec<Parameter> = additional_parameters
            .into_iter()
            .map(|p| Parameter::universal(p.name, p.typ))
            .collect();
    ...
    fn build_functional_axiom_for(&self, prepared: &PreparedBinding) -> Axiom {
    ...
    fn build_total_axiom_for(&self, prepared: &PreparedBinding) -> Axiom {
        let binding = &prepared.binding;
    ...
    /// One `_functional`, one `_total`, and one `{pred}_{idx}` per branch for the given prepared binding.
    fn build_axioms_for(&self, prepared: &PreparedBinding) -> Vec<Axiom> {
        let functional = self
            .build_functional_axiom_for(prepared)
        ...
        let branches = prepared
            ...
            .enumerate()
            .map(|(idx, body_prop)| {
                self.build_branch_axiom_for(prepared, idx, body_prop)
                    .with_suggested_proof()
            });

        std::iter::once(functional)
    ...
    fn branch_axioms(&self) -> Vec<Axiom> {
        self.prepared
            .iter()
    ...
    pub(crate) fn generate_all(&self) -> Vec<Axiom> {
        ...
        axioms.extend(self.branch_axioms());

        for axiom in &axioms {
            axiom.validate().unwrap_or_else(|e| {
    ...
    pub(crate) fn validate_with_lean(
        ...
        validate_lean_code(&lean_code).map_err(|e| {
            let bindings = self
                ...
                .join(", ");
            format!(
                "Lean validation failed for bindings [{}]: {}\n--- generated Lean code ---\n{}",
                bindings, e, lean_code
            )
        })
    }
}
...
mod tests {
    ...
    fn test_sorted_axiom_structures() {
        ...
        let sorted_0_expected = "theorem sorted_0 : ∀ l : ilist, ∀ res : Bool, (((is_nil l) ∧ (true = res)) → (sorted l res)) := by grind";
        let sorted_1_expected = "theorem sorted_1 : ∀ l : ilist, ∀ x : Int, ∀ xs : ilist, ∀ res : Bool, ((((is_cons l) ∧ ((head l) = x) ∧ ((tail l) = xs)) ∧ (is_nil xs) ∧ (true = res)) → (sorted l res)) := by grind";
        let sorted_2_expected = "theorem sorted_2 : ∀ l : ilist, ∀ x : Int, ∀ xs : ilist, ∀ y : Int, ∀ ys : ilist, ∀ res_0 : Bool, ∀ res : Bool, ((((is_cons l) ∧ ((head l) = x) ∧ ((tail l) = xs)) ∧ ((is_cons xs) ∧ ((head xs) = y) ∧ ((tail xs) = ys)) ∧ (sorted xs res_0) ∧ ((((x ≤ y) ∧ res_0) ∧ res) ∨ (¬(((x ≤ y) ∧ res_0)) ∧ ¬(res)))) → (sorted l res)) := by grind";
    ...
    fn test_generate_axioms_from_mem_function() {
        ...
        let mem_0_expected = "theorem mem_0 : ∀ l : ilist, ∀ x : Int, ∀ res : Bool, (((is_nil l) ∧ (false = res)) → (mem l x res)) := by grind";
        let mem_1_expected = "theorem mem_1 : ∀ l : ilist, ∀ x : Int, ∀ h : Int, ∀ t : ilist, ∀ res_0 : Bool, ∀ res : Bool, ((((is_cons l) ∧ ((head l) = h) ∧ ((tail l) = t)) ∧ (mem t x res_0) ∧ ((((h = x) ∨ res_0) ∧ res) ∨ (¬(((h = x) ∨ res_0)) ∧ ¬(res)))) → (mem l x res)) := by grind";
    ...
    fn test_generate_axioms_from_all_eq_function() {
        ...
        let all_eq_0_expected = "theorem all_eq_0 : ∀ l : ilist, ∀ x : Int, ∀ res : Bool, (((is_nil l) ∧ (true = res)) → (all_eq l x res)) := by grind";
        let all_eq_1_expected = "theorem all_eq_1 : ∀ l : ilist, ∀ x : Int, ∀ h : Int, ∀ t : ilist, ∀ res_0 : Bool, ∀ res : Bool, ((((is_cons l) ∧ ((head l) = h) ∧ ((tail l) = t)) ∧ (all_eq t x res_0) ∧ ((((h = x) ∧ res_0) ∧ res) ∨ (¬(((h = x) ∧ res_0)) ∧ ¬(res)))) → (all_eq l x res)) := by grind";
    ...
    fn test_generate_axioms_from_lower_bound_function() {
        ...
        let lb_0_expected = "theorem lower_bound_0 : ∀ t : tree, ∀ x : Int, ∀ res : Bool, (((is_leaf t) ∧ (true = res)) → (lower_bound t x res)) := by grind";
        let lb_1_expected = "theorem lower_bound_1 : ∀ t : tree, ∀ x : Int, ∀ y : Int, ∀ l : tree, ∀ r : tree, ∀ res_0 : Bool, ∀ res_1 : Bool, ∀ res : Bool, ((((is_node t) ∧ ((value t) = y) ∧ ((left t) = l) ∧ ((right t) = r)) ∧ (lower_bound l x res_0) ∧ (lower_bound r x res_1) ∧ ((((x ≤ y) ∧ (res_0 ∧ res_1)) ∧ res) ∨ (¬(((x ≤ y) ∧ (res_0 ∧ res_1))) ∧ ¬(res)))) → (lower_bound t x res)) := by grind";
    ...
    fn test_generate_axioms_from_upper_bound_function() {
        ...
        let expected = "theorem upper_bound_0 : ∀ t : tree, ∀ x : Int, ∀ res : Bool, (((is_leaf t) ∧ (true = res)) → (upper_bound t x res)) := by grind";
    ...
    fn test_len_1_axiom_structure() {
        ...
        let len_1_expected = "theorem len_1 : ∀ l : ilist, ∀ x : Int, ∀ xs : ilist, ∀ res_0 : Int, ∀ res : Int, ((((is_cons l) ∧ ((head l) = x) ∧ ((tail l) = xs)) ∧ (len xs res_0) ∧ ((1 + res_0) = res)) → (len l res)) := by grind";
    ...
    fn test_height_existential_scoping_and_structures() {
        ...
        let height_1_expected = "theorem height_1 : ∀ t : tree, ∀ v : Int, ∀ l : tree, ∀ r : tree, ∀ res_0 : Int, ∀ res_1 : Int, ∀ res : Int, ((((is_node t) ∧ ((value t) = v) ∧ ((left t) = l) ∧ ((right t) = r)) ∧ ((height l res_0) ∧ (height r res_1) ∧ (res_0 > res_1)) ∧ (height l res_0) ∧ ((1 + res_0) = res)) → (height t res)) := by grind";

        let height_2_expected = "theorem height_2 : ∀ t : tree, ∀ v : Int, ∀ l : tree, ∀ r : tree, ∀ res_0 : Int, ∀ res_1 : Int, ∀ res : Int, ((((is_node t) ∧ ((value t) = v) ∧ ((left t) = l) ∧ ((right t) = r)) ∧ ((height l res_0) ∧ (height r res_1) ∧ ¬((res_0 > res_1))) ∧ (height r res_1) ∧ ((1 + res_1) = res)) → (height t res)) := by grind";
  ...
  if x = 0 then true else
      ...
      if x = (0-1) then false else
        ...
        assert_axiom_lean_output(
            ...
            &[
                (
                    "is_even_0",
                    "theorem is_even_0 : ∀ x : Int, ∀ res : Bool, (((x = 0) ∧ (true = res)) → (is_even x res)) := by grind",
                ),
                (
                    "is_even_1",
                    "theorem is_even_1 : ∀ x : Int, ∀ res : Bool, ((¬((x = 0)) ∧ (x = 1) ∧ (false = res)) → (is_even x res)) := by grind",
                ),
                (
                    "is_even_2",
                    "theorem is_even_2 : ∀ x : Int, ∀ res : Bool, ((¬((x = 0)) ∧ ¬((x = 1)) ∧ (x = (0 - 1)) ∧ (false = res)) → (is_even x res)) := by grind",
                ),
                (
                    "is_even_3",
                    "theorem is_even_3 : ∀ x : Int, ∀ res_0 : Bool, ∀ res : Bool, ((¬((x = 0)) ∧ ¬((x = 1)) ∧ ¬((x = (0 - 1))) ∧ (x > 1) ∧ (is_even (x - 2) res_0) ∧ (res_0 = res)) → (is_even x res)) := by grind",
                ),
                (
                    "is_even_4",
                    "theorem is_even_4 : ∀ x : Int, ∀ res_1 : Bool, ∀ res : Bool, ((¬((x = 0)) ∧ ¬((x = 1)) ∧ ¬((x = (0 - 1))) ∧ ¬((x > 1)) ∧ (is_even (x + 2) res_1) ∧ (res_1 = res)) → (is_even x res)) := by grind",
                ),
            ],
        );
    ...
    | Cons { head = x; tail = xs } ->
      is_even_num x && all_even xs";
        ...
        let all_even_0_expected = "theorem all_even_0 : ∀ l : ilist, ∀ res : Bool, (((is_nil l) ∧ (true = res)) → (all_even l res)) := by grind";
        let all_even_1_expected = "theorem all_even_1 : ∀ l : ilist, ∀ x : Int, ∀ xs : ilist, ∀ res_2 : Bool, ∀ res_3 : Bool, ∀ res : Bool, ((((is_cons l) ∧ ((head l) = x) ∧ ((tail l) = xs)) ∧ (is_even_num x res_2) ∧ (all_even xs res_3) ∧ (((res_2 ∧ res_3) ∧ res) ∨ (¬((res_2 ∧ res_3)) ∧ ¬(res)))) → (all_even l res)) := by grind";
