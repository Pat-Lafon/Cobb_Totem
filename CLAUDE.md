...
**Proof Tactics** (generated by `Axiom::generate_proof_tactic` in `spec_ir.rs`):
- No existential quantifiers → `grind`.
- One or more existentials → a case-analysis block: `try aesop (config := { maxRuleHeartbeats := 20000 })`, then `intros`/`cases` on the first `Named`/`Int` parameter (the main data structure), then a generic `| _ =>` arm closing with `try simp_all; grind` / `try aesop` / `try grind`.
